...
# Disease definitions (example: include as many as needed)
DISEASES = [
    {
        'key': 'fmd',
...
def init_state():
    if 'stage' not in st.session_state:
        st.session_state.stage = 'select'  # select, question, or result
...
def show_result():
    disease = st.session_state.selected
    # Evaluate if all answers positive
    ...
    if st.button('Restart'):
        for key in ['stage', 'selected', 'responses', 'index', 'region']:
...
def main():
    ...
    if st.session_state.stage == 'select':
        ...
        choice = st.selectbox('Select Disease', [''] + [d['name'] for d in DISEASES])
        if choice and st.button('Start Diagnosis'):
            st.session_state.selected = next(d for d in DISEASES if d['name'] == choice)
